...
const Projects = () => {
  const { state, fetchProjects } = usePortfolio();
  ...
  const selectedProjectData = state.projects.find(p => p.id === selectedProject);
  const totalPages = Math.ceil(state.projects.length / itemsPerPage);
  const currentProjects = state.projects.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {state.projects.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-muted-foreground text-lg">Belum ada portfolio yang tersedia.</p>
          </div>
        ) : (
          <>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-muted/20 rounded-xl p-4 border border-border/30">
              <p className="text-sm text-muted-foreground text-center">
                📧 Hubungi kami untuk informasi lebih lanjut tentang project ini
              </p>
            </div>
          </div>
        </DialogContent>
